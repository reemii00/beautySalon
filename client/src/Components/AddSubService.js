...
const SubServiceForm = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await Axios.get('http://localhost:8000/dropdown/services'); // Update with your API endpoint
  ...
  const SubmitHandler = async (e) => {
    ...
    try {
      const response = await Axios.post(`http://localhost:8000/${selectedServiceId}/sub-service`, formData, {
        headers: {
  ...
  return (
    <div className="h-screen mx-auto p-6 bg-white shadow-md rounded-md">
      <h2 className="text-2xl font-bold mb-4">Add New Sub Service</h2>
      <label>Select a Service</label>
      <select onChange={handleServiceChange} defaultValue="" style={{ padding: '8px', width: '100%' }}>
